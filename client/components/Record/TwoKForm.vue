...
const submitRecord = async (recordTime: string) => {
  try {
    await fetchy("/api/prs/twok", "POST", {
      body: { pr: recordTime },
    });
  } catch (_) {
    return;
...
<template>
  <form class="record-form">
    ...
    <div class="form-group">
      ...
      <input type="text" id="recordTime" v-model="recordTime" placeholder="MM:SS" required />
    </div>
    <button type="submit" @click.prevent="submitRecord(recordTime)">Submit</button>
  </form>
</template>
<style scoped>
